...
if (version_compare($wp_version, RML_MIN_WP, '>=')) {
    ...
    // Load core
    if ($load_core) {
        require_once(RML_INC . 'general/Core.class.php');
        call_user_func(array(RML_NS . '\\general\\Core', 'getInstance'));
    }else{
        // WP REST API version not reached
        require_once(RML_INC . 'others/fallback-rest-api.php');
    }
}else{
    // Min WP version not reached
    require_once(RML_INC . 'others/fallback-wp-version.php');
}
?>